...
export type LogType = 'critical' | 'error' | 'warn' | 'success' | 'system' | 'info' | 'lock' | 'policy' | 'insight';
...
export const getLogStyle = (type: LogType, isDark: boolean) => {
    ...
    const tailwindColors: Record<LogType, string> = {
        ...
        insight:  isDark 
            ? 'text-sky-400 font-black border-l-2 border-sky-500 pl-2' 
            : 'text-sky-700 font-black border-l-2 border-sky-600 pl-2'
    };
